    ...
    public function dataTable(DataTables $dataTables, $query)
    {
        return datatables($query)
            ...
            ->addColumn('photo', function(User $user){
                $img = !empty($user->photo) ? "<img width='70' height='50' class='img-circle' src='" . it()->url($user->photo)   . "' />" : trans('admin.notfound');
                return $img ;
            })
            ->addColumn('phone', function(User $user){
                $number = !empty($user->phone) ? "<label class='label label-success'>" . $user->phone .  "</label>" : trans('admin.notfound');
                return $number ;
            })
            ...
            ->rawColumns(['checkbox','show_action','actions','user','date', 'phone', 'photo']);
    }
